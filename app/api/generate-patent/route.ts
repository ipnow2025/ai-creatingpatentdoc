...
모든 내용을 자연스러운 문장으로 작성하고, 특허 전문 용어를 사용하세요.`
    ...
    if (responseText.includes("FUNCTION_INVOCATION_TIMEOUT") || responseText.includes("An error occurred")) {
      console.error("[v0] Function timeout detected")
    ...
    if (responseText.trim().startsWith("<") || responseText.trim().startsWith("<!")) {
      console.error("[v0] HTML error page received instead of JSON")
      return NextResponse.json(
        {
    ...
    // responseText is already the generated text string from callGeminiApi
    const generatedText = responseText || "특허 명세서 생성에 실패했습니다."
