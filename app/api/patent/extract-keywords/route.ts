...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!text) {
      console.error("[extract-keywords] Missing text parameter");
      return NextResponse.json({ error: "텍스트가 필요합니다." }, { status: 400 });
    }

    console.log("[extract-keywords] Starting extraction, text length:", text.length);
...
JSON 형식으로만 응답하고, 다른 설명은 포함하지 마세요.`;
    ...
    try {
      ...
      console.log("[extract-keywords] Successfully parsed JSON data");
      ...
      console.error("[extract-keywords] Failed to parse structured data:", parseError);
      console.error("[extract-keywords] Generated text (first 500 chars):", generatedText.substring(0, 500));
      const keywordsMatch = generatedText.match(/\[([\s\S]*?)\]/);
      if (keywordsMatch) {
    ...
    console.error("[extract-keywords] Unhandled error:", error);
